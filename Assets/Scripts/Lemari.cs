...
public class Lemari : MonoBehaviour
{
    ...
    private void CreateVertices() {
        ...
        for (int y = 0; y <= ySize; y++) {
            for (int x = 0; x <= xSize; x++) {
                cubeUV[v] = new Vector2 ((float)x/xSize, (float)y/ySize);
                SetVertex(v++, x, y, 0);
            }
            for (int z = 1; z <= zSize; z++) { 
                cubeUV[v] = new Vector2 ((float)z/zSize, (float)y/ySize);
        ...
        for (int z = 1; z < zSize; z++) {
            for (int x = 1; x < xSize; x++) { 
                cubeUV[v] = new Vector2 ((float)z/zSize, (float)x/xSize);
                SetVertex(v++, x, ySize, z);
            }
        }
        for (int z = 1; z < zSize; z++) {
            for (int x = 1; x < xSize; x++) { 
                cubeUV[v] = new Vector2 ((float)x/xSize, (float)z/zSize);
                SetVertex(v++, x, 0, z);
            }
        }
    ...
    private void CreateTriangles() {
        ...
        int[] trianglesZ1 = new int[(xSize * ySize) * 12]; 
        ...
        for(int y = 0; y < ySize; y++, v++) {
            ...
            for (int q = 0; q < xSize; q++, v++) {
                tZ = SetQuad(trianglesZ1, tZ, v, v + 1, v + ring, v + ring + 1);
            }
        ...
        mesh.SetTriangles(trianglesZ1, 3);
    }
